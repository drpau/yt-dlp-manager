...
function currentChoices() { return availableFormats.filter(format => format.mode === mode); }

function updateAccess() { browserAccess.hidden = sourceValue() !== 'browser'; fileAccess.hidden = sourceValue() !== 'file'; }
function updateRecipe() {
  const selected = currentChoices()[Number(quality.value)];
  document.querySelector('#recipe-mode').textContent = mode === 'video' ? 'Video + audio' : 'Audio only';
  document.querySelector('#recipe-format').textContent = selected ? selected.label : `No ${mode} formats available`;
  qualityOutput.textContent = selected ? selected.label : 'Not available';
  document.querySelector('#download-button').disabled = !selected;
}
function updateQuality() {
  const choices = currentChoices();
  quality.min = '0'; quality.max = String(Math.max(choices.length - 1, 0)); quality.value = String(Math.max(choices.length - 1, 0));
  quality.disabled = choices.length === 0;
  updateRecipe();
}
document.querySelectorAll('input[name="cookie_source"]').forEach(input => input.addEventListener('change', updateAccess));
document.querySelectorAll('input[name="mode"]').forEach(input => input.addEventListener('change', () => { mode = input.value; updateQuality(); }));
quality.addEventListener('input', updateRecipe);
updateAccess(); updateQuality();

form.addEventListener('submit', async event => {
  event.preventDefault();
  showMessage(message, ''); mediaPanel.hidden = true; jobPanel.hidden = true;
  const submit = form.querySelector('button[type="submit"]'); submit.disabled = true; submit.textContent = 'Inspecting…';
  try {
    ...
    document.querySelector('#recipe-title').textContent = media.title;
    document.querySelector('#source').textContent = media.uploader || media.source || 'Supported source';
    document.querySelector('#recipe-source').textContent = media.source || media.uploader || 'Link';
    ...
    showMessage(message, data.cookie_file_loaded ? 'cookies.txt loaded for this workflow. It will be discarded when the job ends.' : '');
    mediaPanel.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
  } catch (error) { showMessage(message, error.message); }
  finally { submit.disabled = false; submit.innerHTML = 'Inspect link <span aria-hidden="true">→</span>'; }
});
...
document.querySelector('#download-button').addEventListener('click', async () => {
  ...
  try {
    const response = await fetch('/api/jobs', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({analysis_token: analysisToken, url: analysedUrl, mode, format_id: selected.id}) });
